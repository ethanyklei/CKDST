...
@dataclass
class S2TDKDWithCTLConfig(LabelSmoothedCrossEntropyCriterionConfig):
    ...
    ctl_weight: float = field(
        default=1.0
    )
    ctl_temperature: float = field(
        default=0.02
    )
    ctl_warmup: int = field(
        default=4000
    )
...
@register_criterion(
    "s2t_dkd_with_ctl", dataclass=S2TDKDWithCTLConfig
)
class S2TDKDWithCTLCriterion(LabelSmoothedCrossEntropyCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.training:
            ...
            ctl_weight = min((model.num_updates / self.ctl_warmup), 1) * self.ctl_weight
            if ctl_weight > 0:
                ...
                ctl_loss = ctl_loss * ctl_weight
        else:
            dkd_weight = 0.0
        
        loss = (1 - dkd_weight) * st_loss + dkd_loss + mt_loss  + ctl_loss
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "ctl_loss", ctl_loss_sum / nsentences / math.log(2), nsentences, round=3
        )
