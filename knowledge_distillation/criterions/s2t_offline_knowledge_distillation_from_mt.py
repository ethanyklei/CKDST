...
from knowledge_distillation.criterions.s2t_online_knowledge_distillation_from_mt import (
    compute_kd_loss
)


@dataclass
class S2TOfflineKnowledgeDistillationFromMTConfig(LabelSmoothedCrossEntropyCriterionConfig):
...
@register_criterion(
    "s2t_offline_knowledge_distillation_from_mt", dataclass=S2TOfflineKnowledgeDistillationFromMTConfig
)
class S2TOfflineKnowledgeDistillationFromMTCriterion(LabelSmoothedCrossEntropyCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.training:
            if self.alpha > 0.0:
                # with torch.no_grad():
                model.teacher.eval()
                mt_tchr_net_output = model.teacher(sample['src_txt_tokens'],
                                sample['src_txt_lengths'],
                                sample['net_input']['prev_output_tokens'])

                kd_loss = self._compute_kd_loss(
                                st_logits=net_output[0], 
                                mt_logits=mt_tchr_net_output[0], 
                                sample=sample, 
                                reduce=reduce)
            else:
                kd_loss = torch.tensor(0.0)
            ...
            if self.mt_weight > 0.0:
                mt_net_output = model(
                                    sample['src_txt_tokens'],
                                    sample['src_txt_lengths'],
                                    sample['net_input']['prev_output_tokens'], mode="text"
                                    )
                mt_loss, _ = self.compute_loss(model, mt_net_output, sample, reduce=reduce)
            else:
                mt_loss = torch.tensor(0.0)

            loss = self.alpha * kd_loss + (1.0 - self.alpha) * st_loss + self.mt_weight * mt_loss
